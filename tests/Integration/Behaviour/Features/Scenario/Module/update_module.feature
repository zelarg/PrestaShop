# ./vendor/bin/behat -c tests/Integration/Behaviour/behat.yml -s module --tags update-module
...
Feature: Module
  ...
  As a BO user
  I must be able to upload/update modules

  Scenario: Update module with zip file from remote (not present initially)
  ...
  Scenario: Update module with zip file from remote (present initially)
    # Check that module is not present before we upload it
    Given module dashproducts has following infos:
      | technical_name    | dashproducts |
      | installed_version | 2.1.4        |
